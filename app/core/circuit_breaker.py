...
from app.errors import GreeksCalculationError, ConfigurationError
...
class GreeksCircuitBreaker:
    ...
    def _get_function_timeout(self, func: Callable) -> float:
        ...
        elif 'bulk' in func_name.lower() or 'chain' in func_name.lower():
            return self.config.greeks_timeout
        else:
            return self.config.individual_timeout
    
    async def _handle_open_circuit(self, fallback_value: Optional[Any], cache_key: Optional[str]) -> Any:
        ...
        if fallback_value is not None:
            log_warning(f"Circuit breaker OPEN: returning fallback value")
        ...
        if cache_key and cache_key in self._fallback_cache:
            ...
            if time.time() - cached_data['timestamp'] < self._cache_ttl:
                log_warning(f"Circuit breaker OPEN: returning cached value")
        ...
        **kwargs
        ...
        try:
            ...
            if self._should_open_circuit():
                self._transition_to_open()
                log_warning(f"Circuit breaker: Failure threshold reached, transitioning to OPEN")
