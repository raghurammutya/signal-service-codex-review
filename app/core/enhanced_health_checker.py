...
class EnhancedHealthChecker:
        ...
        try:
            ...
            return {
                ...
                'details': {
                    ...
                    'external_services': external_health if not isinstance(external_health, Exception) else {'status': ComponentStatus.DOWN.value, 'error': str(external_health)},
    ...
    def _calculate_response_time_score(self, response_time_ms: float) -> int:
        ...
        if response_time_ms <= thresholds['excellent']:
            return 100
        if response_time_ms <= thresholds['good']:
            return 90
        if response_time_ms <= thresholds['acceptable']:
            return 75
        if response_time_ms <= thresholds['poor']:
            return 60
        return 40
...
_enhanced_health_checker: EnhancedHealthChecker | None = None
