...
from app.errors import (
    StreamProcessingError, DataAccessError, ComputationError,
    ProcessingTimeoutError, handle_computation_error, handle_data_access_error
)
from app.schemas.config_schema import (
    SignalConfigData, TickProcessingContext, ComputationResult
)
...
class SignalProcessor:
    ...
    async def execute_configuration(self, config: SignalConfigData, context: TickProcessingContext) -> Optional[ComputationResult]:
        ...
        try:
            ...
            if tasks:
                if config.parallel_execution:
                    ...
                    for (comp_type, _), result in zip(tasks, computation_results):
    ...
    async def get_aggregated_data(self, instrument_key: str, configs: List[SignalConfigData]) -> Optional[Dict]:
    ...
    async def compute_greeks_for_instrument(self, instrument_key: str) -> Optional[Dict]:
    ...
    async def _compute_moneyness_greeks_local(self, moneyness_key: str) -> Optional[Dict]:
    ...
    async def _get_latest_market_data(self, instrument_key: str) -> Optional[Dict]:
