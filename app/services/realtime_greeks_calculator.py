...
class RealTimeGreeksCalculator:
    ...
    async def calculate_realtime_greeks(
        ...
        try:
            ...
            if not option_price:
                log_warning(f"Could not extract option price from tick data")
                return {}
            ...
            if not volatility:
                log_warning(f"Could not get volatility for Greeks calculation")
                return {}
    ...
    def extract_option_details(self, instrument_key: str) -> Optional[Dict]:
        """Extract option details from instrument key"""
        try:
    ...
    def parse_expiry_date(self, expiry_str: str) -> Optional[str]:
    ...
    def extract_option_price(self, tick_data: Dict) -> Optional[float]:
    ...
    async def get_underlying_price(self, underlying_symbol: str, timestamp: datetime) -> Optional[float]:
    ...
    async def get_last_known_underlying_price(self, underlying_symbol: str) -> Optional[float]:
    ...
    async def get_indvix_volatility(self) -> Optional[float]:
