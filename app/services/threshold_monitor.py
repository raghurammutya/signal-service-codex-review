...
class ThresholdConfig:
    ...
    secondary_value: float | None = None  # For BETWEEN/OUTSIDE
...
class ThresholdMonitor:
    ...
    async def check_threshold_on_demand(self, job_id: str) -> dict[str, Any] | None:
    ...
    async def _check_single_threshold(self, job_id: str, job: MonitoringJob) -> dict[str, Any] | None:
    ...
    async def _get_market_data(self, symbol: str, timeframe: str, lookback: int) -> dict[str, Any] | None:
    ...
    def _get_alert_priority(self, config: ThresholdConfig) -> str:
        ...
        if config.financial_impact == FinancialImpact.HIGH and config.time_sensitivity == TimeSensitivity.HIGH:
            return "critical"
        if config.financial_impact == FinancialImpact.MEDIUM or config.time_sensitivity == TimeSensitivity.HIGH:
            return "high"
        if config.financial_impact == FinancialImpact.LOW and config.time_sensitivity == TimeSensitivity.MEDIUM:
            return "medium"
        return "low"
    ...
    def _strategy_to_interval(self, strategy: MonitoringStrategy) -> int:
        ...
        if strategy.tier == MonitoringTier.HIGH_FREQUENCY_PERIODIC:
            # Parse frequency string
            if strategy.frequency:
                if strategy.frequency.endswith('s'):
                    return int(strategy.frequency[:-1])
                if strategy.frequency.endswith('m'):
                    return int(strategy.frequency[:-1]) * 60
            return 30  # Default 30 seconds
        if strategy.tier == MonitoringTier.PERIODIC:
