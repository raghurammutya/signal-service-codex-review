...
from py_vollib.black_scholes.greeks.analytical import rho, theta, vega
...
class GreeksCalculationEngine:
    ...
    def _get_effective_risk_free_rate(self, risk_free_rate: float | None) -> float:
    ...
    async def calculate_all_greeks(
        ...
        option_type: str,
        risk_free_rate: float | None = None,
        greeks_to_calculate: list | None = None
    ) -> dict[str, float | None]:
        ...
        if tasks:
            ...
            for (greek, _), result in zip(tasks, task_results, strict=False):
    ...
    async def calculate_option_chain_greeks(
        ...
        underlying_price: float,
        greeks_to_calculate: list[str] | None = None,
    ...
    async def _legacy_option_chain_calculation(
        ...
        for option_data in option_chain_data:
            try:
                ...
                results.append(dict.fromkeys(greeks_to_calculate))
    ...
    def get_vectorized_performance_metrics(self) -> dict[str, Any] | None:
    ...
    def calculate_time_to_expiry(self, expiry_date: str | date | datetime) -> float:
    ...
    def _validate_greek_output(self, value: float, greek_name: str) -> float | None:
