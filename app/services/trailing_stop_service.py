...
import numpy as np
from app.sdk import InstrumentClient, OrderClient, DataClient, create_instrument_client, create_order_client, create_data_client
from app.sdk import OrderType, OrderSide, TimeFrame
...
class TrailingStopService:
    ...
    async def create_trailing_stop(self,
                                 ...
                                 expires_in_hours: Optional[int] = 24) -> Dict[str, Any]:
        """
    ...
    def _calculate_new_stop_price(self, config: TrailingStopConfig) -> float:
        ...
        elif config.trail_type == TrailingStopType.ABSOLUTE:
            if config.side == "SELL":  # Long position
                return peak_price - config.trail_value
            else:  # Short position
                return peak_price + config.trail_value
                
        elif config.trail_type == TrailingStopType.ATR:
