...
class GreeksCacheManager:
    ...
    async def _should_recalculate_greeks(self, instrument_id: str, market_data: Dict[str, Any], cached_greeks: Optional[Dict]) -> Tuple[bool, GreeksInvalidationContext]:
    ...
    async def _recalculate_greeks(self, request: GreeksRecalculationRequest) -> Dict[str, Any]:
        ...
        try:
            if request.calculation_type == GreeksCalculationType.BULK_CHAIN:
                ...
                result = await self._bulk_greeks_calculation(request.instruments)
            else:
                # Individual calculation
                result = await self._individual_greeks_calculation(request.instruments[0])
    ...
    async def _get_cached_greeks(self, instrument_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _get_market_data(self, instrument_id: str) -> Optional[Dict[str, Any]]:
