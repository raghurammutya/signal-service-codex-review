...
class EnhancedSignalDeliveryService:
    ...
    def _determine_delivery_strategy(
        ...
        elif priority == SignalPriority.HIGH or is_high_return:
    ...
    def _get_smart_fallback_preferences(
        ...
        elif strategy == DeliveryStrategy.FULL_DELIVERY:
    ...
    async def _emergency_fallback_delivery(
        ...
        try:
            emergency_prefs = self._get_smart_fallback_preferences(user_id, DeliveryStrategy.EMERGENCY)
            
            # Attempt minimal viable delivery
            minimal_result = await self.alert_client.send_signal_alert(
                user_id=user_id,
                signal_data=signal_data,
...
_enhanced_delivery_service: Optional[EnhancedSignalDeliveryService] = None
