...
from app.brokers.kite_client import KiteBrokerClient, create_kite_client
...
class BrokerFactory:
    ...
    def get_client(self, broker_type: Optional[BrokerType] = None) -> BaseBrokerClient:
        """
    ...
    def get_default_broker(self) -> Optional[BrokerType]:
...
class MultiBrokerClient:
    ...
    async def get_best_quote(self, instrument_key: str) -> Dict[str, Any]:
        ...
        # Find best bid/ask
        best_bid = max(q for q in valid_quotes if q["bid"] is not None, key=lambda x: x["bid"], default=None)
        best_ask = min(q for q in valid_quotes if q["ask"] is not None, key=lambda x: x["ask"], default=None)
