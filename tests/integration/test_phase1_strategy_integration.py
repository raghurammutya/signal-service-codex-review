...
from app.services.strategy_execution_service import (
    StrategyExecutionService, StrategyConfig, PositionType
)
...
from app.services.trailing_stop_service import (
    TrailingStopService, TrailingStopType, TrailingStopStatus
)
from app.services.alert_service import (
    AlertService, AlertType, AlertCondition, NotificationChannel
)
from app.middleware.metadata_enrichment import (
    MetadataEnrichmentMiddleware, EnrichmentConfig
)
from app.api.v1.enriched_endpoints import StrategyAPIEnriched
from app.sdk import (
    InstrumentClient, OrderClient, DataClient, OrderType, OrderSide,
    create_instrument_client, create_order_client, create_data_client
)
from app.brokers.broker_factory import BrokerFactory, BrokerCredentials, BrokerType
...
class TestPhase1StrategyIntegration:
    ...
    async def test_metadata_enrichment_performance(self, test_services, mock_registry_data):
        ...
        with patch.object(services['clients']['instrument'], 'get_instrument_metadata', side_effect=mock_get_metadata_with_latency):
            ...
            cached_enriched = await enrichment_middleware.enrich_response_data(
    ...
    async def test_concurrent_request_performance(self, test_services, mock_registry_data):
        ...
        with patch.object(services['clients']['instrument'], 'get_instrument_metadata', side_effect=mock_get_metadata):
            ...
            for i in range(20):  # 20 concurrent operations
                operation = services['api'].get_enriched_quote(f"AAPL_NASDAQ_EQUITY")
    ...
    async def test_phase3_sla_compliance(self, test_services, mock_registry_data):
        ...
        with patch.object(services['clients']['instrument'], 'get_instrument_metadata', side_effect=mock_get_metadata):
            ...
            print(f"✅ Phase 3 SLA compliance validated:")
