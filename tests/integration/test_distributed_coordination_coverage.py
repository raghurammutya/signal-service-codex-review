...
class TestDistributedCoordinationQueueGrowth:
    ...
    async def test_coordination_during_network_partitions(self, distributed_coordinator):
        ...
        partition_scenario = {
            "partition_1": ["signal-service-1", "signal-service-2"],  # Can communicate with each other
            "partition_2": ["signal-service-3", "signal-service-4"],  # Can communicate with each other
            "redis_accessible_from": "partition_1"  # Only partition 1 can reach Redis
        }
