...
class TestCORSAllowedOriginsValidation:
    ...
    def test_production_empty_cors_origins_fails_fast(self):
        ...
        with patch.dict(os.environ, {"CORS_ALLOWED_ORIGINS": ""}, clear=True), pytest.raises(ValueError, match="CORS_ALLOWED_ORIGINS must be configured for production environment"):
            get_allowed_origins("production")
    ...
    def test_production_wildcard_origin_fails_fast(self):
        ...
        for wildcard in wildcard_origins:
            with patch.dict(os.environ, {"CORS_ALLOWED_ORIGINS": wildcard}, clear=True), pytest.raises(ValueError, match=f"Wildcard origins not permitted in production: {wildcard}"):
                get_allowed_origins("production")
    ...
    def test_production_mixed_wildcard_origins_fails_fast(self):
        ...
        with patch.dict(os.environ, {"CORS_ALLOWED_ORIGINS": mixed_origins}, clear=True), pytest.raises(ValueError, match="Wildcard origins not permitted in production"):
            get_allowed_origins("production")
...
class TestCORSMiddlewareConfiguration:
    ...
    def test_cors_middleware_configuration_structure(self):
        ...
        with patch.dict(os.environ, {"CORS_ALLOWED_ORIGINS": valid_origins}, clear=True), patch('common.cors_config.CORSMiddleware') as mock_cors_middleware:
            add_cors_middleware(app, "production")
...
class TestCORSSecurityRequirements:
    ...
    def test_cors_configuration_logging(self):
        ...
        with patch.dict(os.environ, {"CORS_ALLOWED_ORIGINS": valid_origins}, clear=True), patch('common.cors_config.logger') as mock_logger:
            add_cors_middleware(app, "production")
    ...
    def test_cors_configuration_error_logging(self):
        ...
        with patch.dict(os.environ, {}, clear=True), patch('common.cors_config.logger') as mock_logger, pytest.raises(ValueError):
            get_allowed_origins("production")
...
class TestCORSDeploymentSafetyNets:
    ...
    def test_production_deployment_cors_validation_check(self):
        ...
        with patch.dict(os.environ, {}, clear=True), pytest.raises(RuntimeError, match="CORS configuration validation failed for production"):
