...
class WeeklyQualityMonitor:
    ...
    def collect_current_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            metrics["files_affected"] = len({v.get("filename", "") for v in violation_details})
    ...
    def generate_executive_summary(self, report: dict[str, Any], trends: dict[str, Any], alert: dict[str, Any] | None) -> str:
        """Generate executive summary report"""
        timestamp = datetime.now()
        metrics = report["quality_metrics"]

        return f"""# Weekly Code Quality Report
...
{'- **Immediate action required** to resolve blocking issues' if metrics['p0_critical_violations'] > 0 else '- Continue regular quality maintenance'}
...
"""
    ...
    def format_trend_details(self, trends: dict[str, Any]) -> str:
        ...
        summary.get("current_total", 0)
        summary.get("previous_total", 0)
...
"""
    ...
    def save_monitoring_data(self, report: dict[str, Any], trends: dict[str, Any],
                           executive_summary: str, alert: dict[str, Any] | None):
        ...
        print("💾 Monitoring data saved to evidence/weekly/")

    def display_results(self, report: dict[str, Any], trends: dict[str, Any], alert: dict[str, Any] | None):
        ...
        if alert:
            print(f"\n🚨 {alert['alert_message']}")
            if metrics['p0_critical_violations'] > 0:
                print("⚡ Action Required: Fix P0 violations immediately!")
