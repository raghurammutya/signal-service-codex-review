...
class CacheReindexValidator:
    ...
    async def _validate_data_integrity(self, cache_entry: Dict[str, Any]) -> List[str]:
        ...
        for field in required_fields:
            if field not in cache_data:
                errors.append(f"Missing required cache data field: {field}")
        ...
        for field in ["last_price", "volume"]:
            if field in cache_data:
                try:
                    value = float(cache_data[field])
                    if value < 0:
                        errors.append(f"Invalid {field}: negative value not allowed")
                except (ValueError, TypeError):
                    errors.append(f"Invalid {field}: must be numerical")
    ...
    async def _perform_cache_lookup(self, cache_key: str) -> Optional[Dict[str, Any]]:
