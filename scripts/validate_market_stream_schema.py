...
class StreamSchemaValidator:
    ...
    async def _validate_single_message(self, message_id: str, message: Dict[str, Any]) -> ValidationResult:
        ...
        for field, expected_type in self.required_fields.items():
            if field not in message:
                ...
                result.errors.append(f"Invalid type for {field}: expected {expected_type}, got {type(message[field])}")
                result.valid = False
    ...
    def _validate_metadata_enrichment(self, message: Dict[str, Any]) -> Tuple[float, List[str]]:
        ...
        for field, expected_type in self.required_metadata_fields.items():
            ...
            for source in metadata_sources:
                if field in source and isinstance(source[field], expected_type):
                    metadata_fields_found += 1
                    field_found = True
                    break
            
            if not field_found:
                errors.append(f"Missing or invalid metadata field: {field}")
    ...
    def _validate_data_fields(self, data: Dict[str, Any]) -> List[str]:
        ...
        for field, expected_type in self.required_data_fields.items():
            if field not in data:
                errors.append(f"Missing data field: {field}")
            elif not isinstance(data[field], expected_type):
                errors.append(f"Invalid type for data.{field}: expected {expected_type}, got {type(data[field])}")
            elif field in ["ltp", "volume"] and data[field] <= 0:
